    ...
    >
      ...
      >
        ...
        <h1 className="text-4xl sm:text-5xl font-extrabold text-center mb-12 bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-400 leading-tight">
          Poder Actual de Innovación: Nuestro Equipo
        </h1>
          ...
          <Card className="bg-gray-800/80 backdrop-blur-sm border-blue-500 overflow-hidden transition-all duration-300 group-hover:shadow-lg group-hover:shadow-blue-500/20">
            <CardHeader>
              <CardTitle className="text-xl sm:text-2xl md:text-3xl text-blue-300 flex items-center">
                <Lightbulb className="mr-2" /> Eficiencia Aumentada por IA
              </CardTitle>
            </CardHeader>
            <CardContent className="flex flex-col sm:flex-row justify-between items-center gap-4">
              <div className="text-6xl sm:text-7xl font-bold text-purple-300 flex items-center">
                <AnimatedNumber value={12} />
                <span className="text-2xl ml-2">Desarrolladores de Élite</span>
              </div>
              <div className="text-center sm:text-right">
                <div className="text-lg sm:text-xl font-semibold text-blue-300">Eficiencia Sin Precedentes</div>
                <div className="text-sm text-gray-400 max-w-xs">
                  Entregando soluciones empresariales más rápido e inteligentemente
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <Card className="bg-gray-800/80 backdrop-blur-sm border-blue-500 h-full transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/20">
              <CardHeader>
                <CardTitle className="flex items-center text-blue-300 text-xl sm:text-2xl">
                  <Code2 className="mr-2" /> Arsenal Técnico Principal
                </CardTitle>
              </CardHeader>
              <CardContent>
                <ul className="space-y-3">
                  {[
                    { icon: Code2, text: "Desarrollo Full-Stack: Expertos en Node.js, PHP" },
                    { icon: Smartphone, text: "Maestría Móvil: Android/iOS Nativo, React Native, Flutter" },
                    { icon: Brain, text: "Potencia en IA/ML: Científicos de Datos, Ingenieros de ML" },
                    { icon: Lightbulb, text: "Equipo de Innovación: Desarrolladores Full-Stack de IA" },
                    { icon: Network, text: "Infraestructura: Expertos en Administración de Redes" },
                  ].map((item, index) => (
                    <motion.li
                      key={index}
            ...
            <Card className="bg-gray-800/80 backdrop-blur-sm border-purple-500 h-full transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/20">
              <CardHeader>
                <CardTitle className="flex items-center text-purple-300 text-xl sm:text-2xl">
                  <Users className="mr-2" /> Alianzas Estratégicas
                </CardTitle>
              </CardHeader>
              <CardContent>
                <ul className="space-y-3">
                  {[
                    { icon: Globe, text: "Dos agencias de desarrollo en India" },
                    { icon: Globe, text: "Una agencia especializada en Filipinas" },
                    { icon: Lightbulb, text: "Enfoque en escalado específico por proyecto" },
                  ].map((item, index) => (
                    <motion.li
                      key={index}
                      initial={{ opacity: 0, y: 10 }}
          ...
          <Card className="bg-gradient-to-br from-blue-900/80 to-purple-900/80 backdrop-blur-sm border-none overflow-hidden transition-all duration-300 group-hover:shadow-lg group-hover:shadow-purple-500/20">
            <CardHeader>
              <CardTitle className="text-2xl sm:text-3xl text-white flex items-center">
                <Sparkles className="mr-2" /> Nuestro Enfoque Único
              </CardTitle>
            </CardHeader>
            <CardContent className="text-gray-200">
              <p className="mb-4 text-lg leading-relaxed">
                Hemos aprovechado la IA para transformar una operación tradicional de 22 personas en un equipo más ágil
                y eficiente de 12 personas que entrega las mismas soluciones de nivel empresarial en menos tiempo.
              </p>
              <p className="text-lg leading-relaxed font-semibold text-blue-300">
                Esto no se trata solo del tamaño del equipo: se trata de maximizar la creatividad humana con la
                eficiencia de la IA.
              </p>
            </CardContent>
          </Card>
